...
const productId = "3a9f4e97-33f8-4409-9a8a-9637b1071912";
...
// 황색4호 (Tartrazine) — CosIng lists color additives by CI code only, not the common name.
{
  const id = await resolveViaCosing("CI 19140");
  await addAliasAndLink("황색4호", id, 45);
  results["황색4호"] = id;
}

// 청색1호 (Brilliant Blue FCF) — same CI-code-only pattern.
{
  const id = await resolveViaCosing("CI 42090");
  await addAliasAndLink("청색1호", id, 46);
  results["청색1호"] = id;
}
