...
const productId = "fd110727-3145-4e06-837f-e0fba8c0227f";
...
{
  const id = await resolveViaCosing("Glycine Soja Seed Extract");
  await addAliasAndLink("돌콩추출물", id, 13);
  results["돌콩추출물"] = id;
}
{
  const id = await resolveViaCosing("Lonicera Japonica Flower Extract");
  await addAliasAndLink("인동덩굴꽃추출물", id, 20);
  results["인동덩굴꽃추출물"] = id;
}
{
  const id = await resolveViaCosing("Gaultheria Procumbens Leaf Extract");
  await addAliasAndLink("파스향나무잎추출물", id, 23);
  results["파스향나무잎추출물"] = id;
}
